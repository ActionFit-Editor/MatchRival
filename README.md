...
시간은 `ActionFit.Time.IClock`, calendar와 24시간 미만의 `calendarDayBoundaryOffset`을 함께 주입합니다. Clock source와 UTC/디바이스 로컬 calendar 선택은 독립적입니다. 양수 offset은 논리 날짜 판정에서 빼고 deadline 변환에서 다시 더하므로 9시간이면 해당 달력의 09:00이 날짜 경계입니다. 기존 생성자는 offset 0을 유지하며, 시작된 deadline과 active legacy 숫자 tick은 mode/zone/offset 변경으로 다시 계산하지 않습니다.
